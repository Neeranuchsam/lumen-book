...
class AuthorsRatingsControllerTest extends TestCase
{
    ...
    public function test_store_can_add_a_rating_to_an_author()
    {
        ...
        $this->post("/authors/{$author->id}/ratings", ['value' => 5], $this->header_request_accept_type);
    ...
    public function test_destroy_can_delete_an_author_rating()
    {
        $author = factory('App\Author')->create();
        $ratings = $author->ratings()->saveMany(
            factory('App\Rating', 2)->make()
        );
    ...
    public function test_destroy_should_not_delete_ratings_from_another_author()
    {
        ...
        $rating = $authors->last()->ratings()->first();

        $this->delete("/authors/{$firstAuthor->id}/ratings/{$rating->id}", [], ['Accept' => 'application/json'])
            ->seeStatusCode(404);
    }
}
